...
import type { Story } from '../pages/api/stories'
      ...
      {open && (
        <div className="mt-3 bg-white/5 border border-border rounded-xl p-4 text-xs text-white/70 space-y-2">
          ...
          <p className="text-yellow-400/80 pt-1">
            Your session ID stays on this device only — we never store it.
          </p>
        </div>
      )}
    </div>
...
function StoryCard({ story, username }: { story: Story; username: string }) {
  ...
  function download() {
    ...
    const proxyUrl = `/api/proxy?url=${encodeURIComponent(story.videoUrl)}&filename=${encodeURIComponent(filename)}`
